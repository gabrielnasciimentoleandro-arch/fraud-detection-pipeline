...
- V1–V28 são **PCA anônimo**: nenhuma explicabilidade por feature é possível.
- Amount está em **dólares** e o dataset cobre **48 horas**; não há teste fora dessa janela.
- 1.081 linhas **exatamente duplicadas** existem e não foram removidas: em fraude, reprocessar a mesma transação é plausível.
- Recall de 76% = **~1 em cada 4 fraudes passa**. Triagem, não aprovação automática.

## 7. Próximos passos

- Validação **walk-forward** em 3 janelas de tempo, em vez de um corte único.
- Limiar por **orçamento de revisão** (maximizar recall com N analistas/dia), não por F1.
- focal loss ou LightGBM com is_unbalance como segunda opinião.
- Score agregado **por cliente** (janela móvel), não só por transação isolada.
- Monitoramento de **PSI nas features V** para gatilho de retreino.
